...
2. 如需修正，在 `manual/glossary/` 下创建修正记录文件（`术语名.md`）
...
6. 如审查后认为无需修正，创建空标记文件：
   ```
   touch manual/glossary/YYYY-MM-DD_主题.ok
   ```

系统通过检查 `manual/glossary/` 下有无对应课程的 `.ok` 或 `.md` 来判断审查状态。
...
1. 读取 manual/glossary/ 下所有修正记录（`.md` 文件），以 manual 定义为最高优先级
2. 跳过 `.ok` 标记文件（空文件，表示已审查无需修正）
3. 读取 glossary/*.glossary.md 中本次新增的自动术语
4. 与 glossary/current/glossary.md 融合
