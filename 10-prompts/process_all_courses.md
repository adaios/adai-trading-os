...
检测方式：对比 `processed/*.done`，无对应标记的即为新增课程。

处理规则：

1. 检查 processed/ 中是否存在对应 .done 文件
2. 如存在：跳过。
3. 如不存在：继续处理。
...
### ⚠️ 增量验证（Step 3 → Step 5 之间执行）

Step 3 完成后，必须验证 glossary 增量更新是否到位：
...
4. 如审查后认为无需修正，创建空文件：
   ```
   touch manual/glossary/YYYY-MM-DD_主题.md
   ```
5. 不修改 `glossary/*.glossary.md` 原始文件——修正记录在 Step 5 融合时自动覆盖
6. **跨课程文件**（如 `特色词汇.md`）不受日期命名约束
...
1. 读取 manual/glossary/ 下所有 `.md` 文件，跳过 `特色词汇.md` 等非日期命名的跨课程文件（这些由 fusion 逻辑特殊处理）
2. 检查每门新课是否在 manual/glossary/ 有对应的 `YYYY-MM-DD_主题.md` 文件
   - 有 → 已审查，正常融合
   - 无 → **不允许执行 fusion，提示先完成审查**
3. 以 manual 定义为最高优先级
4. 读取 glossary/*.glossary.md 中本次新增的自动术语
5. 与 glossary/current/glossary.md 融合
6. 输出到 glossary/current/glossary.md
...
**⚠️ 执行前验证审查标记：** 本次课程的 `YYYY-MM-DD_主题.md` 必须在 `manual/glossary/` 中存在（无论空还是有内容），否则不允许执行 fusion。
...
**⚠️ 执行前验证：** 本次课程的 `*.rules.md` 文件必须已全部就位。

```
for f in rules/YYYY-MM-DD_主题.rules.md; do
  if [ ! -f "$f" ]; then echo "❌ 缺失: $f"; fi
done
# 全部存在才允许进入 Step 6
```
