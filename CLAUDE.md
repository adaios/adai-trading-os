...
* glossary/
  交易术语库（按课程归档）。
  自动提取术语并生成定义。
  每个文件对应一堂课，只记录该课首次提出的术语定义。
  后来课程对已有术语的补充，写入后来课程自己的文件（不追加旧文件）。
...
* 自动归类
* 自动补充定义
* **不追回到旧文件**：后来课程补充的术语更新，写入后来课程自己的文件，fusion 时会合并到 glossary/current/ 中
...
**Step 3 glossary 输出格式要求：**
每个 glossary 文件末尾须带 `## 待确认` 区块，列出 AI 对定义不确定、需要人工核对的术语：
...
| 术语 | 原因 |
|:----|:-----|
| XXX | 原文表述较模糊，AI 推断的定义可能不准确 |
| YYY | 该术语与现有术语定义有重叠，需确认是否独立术语 |
```

只有 AI 确实不确定的术语才列入，确认无误的术语不写进去。如无待确认项，写 `（无）`。人工审核时优先核对此表。
...
# 有输出 → 表示本次课程对已有术语做了补充
# 不追回到旧文件！补充内容保留在本课自己的文件中，fusion 时会合并到 glossary/current/
```
