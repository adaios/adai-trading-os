...
检测方式：扫描 temp/ 下所有日期目录，与 `processed/*.done` 对比，
无对应 .done 标记的即为新目录。
...
Step 3 完成后，审查 AI 新提取的术语是否有误：
1. 打开新生成的 `glossary/YYYY-MM-DD_主题.glossary.md`，检查术语定义是否准确
2. 如需修正，在 `manual/glossary/YYYY-MM-DD_主题.md` 中添加修正内容
3. 参考已有记录格式（如 `2025-04-23_大富翁导论.md` 含多个术语修正）
4. 如审查后认为无需修正，创建空文件 `manual/glossary/YYYY-MM-DD_主题.md`
...
**前置条件：** 所有已处理课程（`processed/*.done` 存在）必须已在 `manual/glossary/` 完成人工审查（有对应的 `YYYY-MM-DD_主题.md` 文件），否则不允许执行收敛。
