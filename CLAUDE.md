...
3. ✅ `manual/rules/` 下每门课都有对应的 `YYYY-MM-DD_主题.md` 文件（已审查）
4. ✅ `manual/system/` 下每门课都有对应的 `YYYY-MM-DD_主题.md` 文件（已审查）

> 人工审查是收敛的前置质量门，确保所有术语定义、规则和系统条目已人工确认后才允许合并收敛。
> `manual/rules/` 和 `manual/system/` 的审查标记机制与 glossary 一致：有修正写内容，无需修正创建空 `.md` 文件。
